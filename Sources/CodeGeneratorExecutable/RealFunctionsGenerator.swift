enum RealFunctionsGenerator {
    static func realFunctionsExtension(objectType: String, type: String, whereClause: Bool, simdAccelerated: Bool) -> String {
        let elementaryFunctions = [
            ...
            RealFunction(
                name: "pow",
                simdName: "pow",
                arguments: [.init(name: "x", label: "_"), .init(name: "n", label: "_", type: "Int")]
            ),
    ...
    static func realFunctionTemplate(for function: RealFunction, type: String, simdAccelerated: Bool) -> String {
        ...
        let acceleratedArguments = function.arguments
            .map { arg in "\(arg.type.map { _ in ".init(repeating: .init(\(arg.name)))" } ?? arg.name)" }.joined(separator: ", ")
